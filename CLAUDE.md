...
### ⚠️ `worker.js` es un bundle generado, y no hay script que lo genere

`wrangler.toml` apunta a `main = "worker.js"`, un bundle de `src/*.ts` **commiteado a mano**.
`npm run build` corre `tsc` para el server de Express, **no** regenera `worker.js`. O sea:

> **Editar `src/` NO cambia producción, y no da ningún síntoma.**

Al tocar la lógica hay que editar **los dos**: `src/services/finanzas.ts` (el fuente que se lee)
y `worker.js` (el que corre). Falta un `npm run build:worker` con esbuild que cierre esto.
